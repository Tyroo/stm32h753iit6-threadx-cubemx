...
#include   "gx_display_driver_stm32h7_24xrgb.h"
#include   "bsp.h"
...
static void gx_chromeart_canvas_copy(GX_CANVAS *canvas, GX_CANVAS *composite)
{
    ...
    if (_gx_utility_rectangle_overlap_detect(&dirty, &composite -> gx_canvas_dirty_area, &overlap))
    {
		...
		DMA2D->FGPFCCR = (0xff000000) | LTDC_PIXEL_FORMAT_ARGB8888; 		
                                ...
                                INT xpos, INT ypos, GX_PIXELMAP *pixelmap)
{
	...
	{
		DMA2D->FGPFCCR = (0xff000000) | DMA2D_ARGB8888; 
		DMA2D_Mode = DMA2D_M2M_BLEND;       
	}
	else
	{
		DMA2D->FGPFCCR = (0xff010000) | DMA2D_ARGB8888;  
                                ...
                                INT xpos, INT ypos, GX_PIXELMAP *pixelmap, UCHAR alpha)
{
    ...
    if (pixelmap->gx_pixelmap_flags & GX_PIXELMAP_ALPHA)
    {
        /*原始前景层图像的 alpha 通道值替换为当前配置的与原始 alpha 通道值的乘积 */
        blend_reg |= 0x00020000 | LTDC_PIXEL_FORMAT_ARGB8888;
    }
    else
    {
        /* 原始背景层图像的alpha通道值替换为当前配置的alpha值 */
        blend_reg |= 0x00010000 | LTDC_PIXEL_FORMAT_ARGB8888;
    }
                                         ...
                                         GX_POINT *map_offset, const GX_GLYPH *glyph)
{
	...
	DMA2D->BGOR = canvas->gx_canvas_x_resolution - width;  
	DMA2D->BGPFCCR = 0xFF010000;	              /* 原始背景层图像的 alpha 通道值替换为0xFF，ARGB8888 */
...
/*
*********************************************************************************************************
*	函 数 名: stm32h7_24xrgb_buffer_toggle
